...
RSpec.describe Coelacanth do
  ...
  describe "#analyze" do
    ...
    let(:response_metadata) do
      instance_double(
        Coelacanth::HTTP::ResponseMetadata,
        status_code: 200,
        status_message: "OK",
    ...
    let(:http_response) do
      double(
        ...
        body: binary_html,
        code: "200",
        coelacanth_metadata: response_metadata
      )
    end
    ...
    context "when the HTTP request succeeds" do
      before do
        ...
        allow(extractor).to receive(:call) do |**args|
          ...
          expect(args[:response_metadata]).to eq(response_metadata)
          extraction_payload
        end
      end
    ...
    context "when the HTTP request times out" do
      before do
        ...
        allow(extractor).to receive(:call).with(html: "", url: url, response_metadata: nil)
                                        .and_return(extraction_payload.merge(response: nil))
      end
      ...
      it "continues with empty HTML" do
        ...
        expect(result).to eq({
          dom: "parsed_dom",
          screenshot: screenshot,
          extraction: extraction_payload.merge(response: nil),
          response: nil
        })
      end
    end
