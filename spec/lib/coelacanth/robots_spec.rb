...
RSpec.describe Coelacanth::Robots do
  ...
  let(:query_uri) { URI("https://example.com/public?session=123") }
  ...
  it "respects disallow rules that include query strings" do
    stub_request(:get, "https://example.com/robots.txt").to_return(status: 200, body: <<~ROBOTS)
      User-agent: *
      Disallow: /*?session=
    ROBOTS

    expect(described_class.allowed?(query_uri)).to be(false)
  end
